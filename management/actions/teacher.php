...
if(isset($_POST['action']) && $_POST['action'] == 'saveTeacher') {
    ...
    // Handle file uploads
    $upload_dir = 'classroom/uploads/profile/teachers/';
    if (!is_dir($upload_dir)) {
        mkdir($upload_dir, 0777, true);
    }
    
    // กำหนดไฟล์ที่ต้องการอัปโหลด
    $file_fields = ['teacher_image_profile', 'teacher_card_front', 'teacher_card_back', 'teacher_attach_document'];
    $file_paths = [];
    
    foreach ($file_fields as $field) {
        if (isset($_FILES[$field]) && $_FILES[$field]['error'] == UPLOAD_ERR_OK) {
            $image_ext = pathinfo($_FILES[$field]['name'], PATHINFO_EXTENSION);
            $new_file_name = $field . '_' . time() . '.' . $image_ext;
...
if(isset($_POST['action']) && $_POST['action'] == 'getTeacherData') {
    ...
    if (isset($teacherData['teacher_image_profile'])) {
